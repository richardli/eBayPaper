...
######################
START=0
NT.grid=70   #70 #2016=7*24*60/5# #672=7*24*60/15# #1440=24*60#
...
U=matrix(0,12,length(EBAY.training))
for(i in 59:70){
  for(j in 1:length(EBAY.training)){
    U[i-58,j]=fitD$residuals[(i-59)*length(EBAY.training)+j]
  }
}
...
MAPE1st=rep(0,5) #10 #144 #48 #288 is starting from 6.8, 1 min step
MAPE2nd=rep(0,5) #10 #144 #48
Forecast=NULL
Y=NULL;
Forecast=NULL
...
for(i in 60:69){
  ...
  Forecast[[i-59]] = lm(Y~condition+reserve+open+seller+value+bidder+early+jump+currentnumber+Dy+Y.auto)
}

predstep=NULL
real1st=NULL
real2nd=NULL           #last two are from day 6.5
MAPE1st=rep(0,10) #10 #144 #48 #288 is starting from 6.8, 1 min step
MAPE2nd=rep(0,10)
...
for(i in 60:69){ #60:69  #1872:2015  #624:671 #1152:1439 wrt 288
...
ptm <- proc.time()    
    ...
    prev=pred
    pred=predict(Forecast[[i-59]],new)
...
proc.time() - ptm
  ...
  predstep[[i-59]]=as.vector(pred.temp)
  real1st[[i-59]]=real1st.temp
  real2nd[[i-59]]=real2nd.temp
  MAPE1st[i-59]=mean(abs((real1st[[i-59]]-predstep[[i-59]])/real1st[[i-59]]))
  MAPE2nd[i-59]=mean(abs(real2nd[[i-59]]-predstep[[i-59]])/real2nd[[i-59]])
}
...
plot(seq(0,7,len=70),smoothF.2[[i]],type="l",main="Spline Smoothing of Price Curve",xlab="time",ylab="price")
points(EBAY.testing[[i]]$Time,EBAY.testing[[i]]$Price)
