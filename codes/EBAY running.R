...
ave.bw <- 0.7
r <- -1.089
#source('~/Dropbox/Ebay_2012/R_Code_Used_&_Data/EBAY preparing.R', chdir = TRUE)
...
# pdf(paste("fig-allsig", ave.bw, ".pdf"), width=10, height=12 )
...
# }
# dev.off()
# save.image(paste("~/Dropbox/allreg", ave.bw, ".RData"))
# quit(save = "no")
#############################################
...
mape2.fda <- MAPE2nd
save.image(paste("cv", version, "bw",ave.bw, ".RData"))
######################################################################
...
pdf(paste("MAPE-final", version, ".pdf"), width = 6, height = 6)
