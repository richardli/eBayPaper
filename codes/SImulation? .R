...
simFromDens <- function(lambda, folder){
	...
	par(mfrow = c(1,3) )
	plot(plotpoints, base.x1(plotpoints)/scale.coef, main = "x1", type = "l",
		ylim = bestplotRange(base.x1(plotpoints)/scale.coef, coef[,1], 
				            confint[, 1, 1], confint[, 1, 2]), xlab="time")			            
	lines(plotpoints, coef[, 1], type = "l", col = "red")
	lines(plotpoints, confint[, 1, 1], type = "l", col ="grey")
	lines(plotpoints, confint[, 1, 2], type = "l", col ="grey")
	
	plot(plotpoints, base.z1(plotpoints)/scale.coef, main = "z1", type = "l",
	...
	lines(plotpoints, confint[, 2, 1], type = "l", col ="grey")
	lines(plotpoints, confint[, 2, 2], type = "l", col ="grey")
	
	plot(plotpoints, alpha(plotpoints)/scale.coef, main = "self", type = "l",
		ylim = bestplotRange(base.x1(plotpoints)/scale.coef, coef[,3], 
				            confint[, 3, 1], confint[, 3, 2]), xlab="time")
	lines(plotpoints, coef[, 3], type = "l", col = "red")
	lines(plotpoints, confint[, 3, 1], type = "l", col ="grey")
	lines(plotpoints, confint[, 3, 2], type = "l", col ="grey") 
	dev.off()
	####################
	MAPE.beta <- rep(0, 3)
...
beta = 1
par(mfrow = c(3, 2))
alpha <- function(t){10*exp(.1*(t-3)^2) + .5}#}
plot.curv(alpha)
#base.x1 <- function(t){  5 * t^3 - 2 * t }
#base.x1 <- function(t){1}
base.x1 <- function(t){40 - 5*t}#2*t^3 - 2.4*t^2 + 0.54*t + 0.2}#2*t^3 - 2.4*t^2 + 0.54*t + 0.2 }10*cos(t /3 + 5) + 10
plot.curv(base.x1)
#base.x2 <- function(t){  -exp(2 * t) + 3 }
base.x2 <- function(t){0}# cos(t * 3 + 5) + 1}
plot.curv(base.x2)
#base.z1 <- function(t){ t^3 - 0.05 * t^2 + 0.2 * t + 0.5 * cos(t*2)}
base.z1 <- function(t){0.3*(t^2 -0.04*t)}
# 15
#10*cos(t /3 + 5) + 10}
# }#* t^2 -0.04*t}
plot.curv(base.z1)
